export const OPEN_MENU = 'OPEN_MENU'

export const SELECT_MENUITEM = 'SELECT_MENUITEM'

export const FETCH_TOPICS = 'FETCH_TOPICS'
export const NEW_TOPIC = 'NEW_TOPIC'
export const FETCH_COZES = 'FETCH_COZES'
export const NEW_COZE = 'NEW_COZE'
export const VIEW_TOPIC = 'VIEW_TOPIC'
export const JOIN_TOPIC = 'JOIN_TOPIC'
export const FETCH_TOPIC_IN_ARRAY = 'FETCH_TOPIC_IN_ARRAY'
export const REPORT = 'REPORT'
export const FOLLOW_TOPIC = 'FOLLOW_TOPIC'

export const OTHEZ = 'OTHEZ'
export const FETCH_NOTICE = 'FETCH_NOTICE'
export const READ_NOTICE = 'READ_NOTICE'
export const LIKE_COZE = 'LIKE_COZE'

export const SIGNUP_FEZ = 'SIGNUP_FEZ'
export const FETCH_USER = 'FETCH_USER'
export const LOCATE_FEZ = 'LOCATE_FEZ'
export const UPDATE_FEZ = 'UPDATE_FEZ'
export const FETCH_FEZ_FOLLOWED = 'FETCH_FEZ_FOLLOWED'
export const FETCH_FEZ_CREATED = 'FETCH_FEZ_CREATED'


export const TOPIC_NEXT_PAGE = 'TOPIC_NEXT_PAGE'
export const COZE_NEXT_PAGE = 'COZE_NEXT_PAGE'
export const LOADING_NEXT_PAGE = 'LOADING_NEXT_PAGE'


export const FEEDBACK = 'FEEDBACK'